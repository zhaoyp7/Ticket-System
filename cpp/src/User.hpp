...
public:
  UserSystem() : user_data_pos("../database/UserDataPos") {
    user_data.initialise("../database/UserData");
  ...
  int login(const std::string &user, const std::string &pw) {
    ...
    User now;
    user_data.read(now, now_user_pos);
    if (strcmp(now.password, pw.c_str())) {
      return -1;
    }
    if (login_stack[now_user]) {
      return 0;
    }
  ...
  void query_profile(const std::string &cur_user, const std::string &user) {
    ...
    sjtu::vector<int> tmp_now = user_data_pos.find(now_user, INT_MIN);
    if (tmp_now.empty()) {
      std::cout << "-1" << '\n';
      return;
    }
    int now_user_pos = tmp_now[0];
    User now;
    user_data.read(now, now_user_pos);
